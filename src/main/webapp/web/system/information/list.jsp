...
formSubmit('/information/selectModel.do','submit(search)','json',function(data) {
...
function openAdd(){
	openLayer('/web/system/information/add.jsp',refresh)
}
function openUpd(code){
	openLayer('/web/system/information/upd.jsp?code='+code,refresh)
}
function openPic(code){
	openLayer('/web/system/information/pic.jsp?code='+code,refresh)
}
function del(code){
	openConfirm(function(index){
		ajax('/information/del.do', {code:code}, 'text', function(data){
	        if (data == 1) {
	            layer.msg('删除成功');
...
<td>{{ d.count }}</td><td>{{ d.state }}</td><td>{{ d.classCode }}</td>
<td>
