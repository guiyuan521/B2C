...
<body>
    ...
    <form class="layui-form" lay-filter="updsp" method="post">
			<div class="layui-form-item">
				...
				<div class="layui-input-inline">
					<select name="classCode" lay-filter="parentCode2"></select>
				</div>
			</div>
        <div class="layui-form-item">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
               	<select name="state" layui-search>
...
function getSelect(selectid,parentCode){
	...
	ajax('/classification/select.do', {parentCode:parentCode}, 'json', function(data) {		
		var html = '';
...
function init(){
	...
	$.ajax({
        url:con.app+'/information/search.do',
        data : {code:code},
        dataType : 'json',
        type : 'post',
        success : function(data) {
        	ajax('/classification/select.do', {parentCode:data.code1}, 'json', function(data11) {		
        		var html = '';
        		$.each(data11, function(i, dom) {
        			html += "<option value='"+dom.code+"'>"+dom.name+"</option>";
        		})
        		$("select[name='classCode']").html(html);
        		form.render('select');//刷新select选择框渲染
        		form.val("updsp", 
        			{code: data.code,
        				name: data.name,
        				...
        				parentCode:data.code1})
        	})
        }
    })
}
formSubmit('/information/upd.do', 'submit(upd)', 'text', function(data) {
	if (data == 1) {
        layer.msg('修改成功');
        closeThis(1000);
    } else {
